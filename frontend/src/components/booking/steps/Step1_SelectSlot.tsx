      ...
      <div className="flex flex-col xl:flex-row gap-6 flex-1 min-h-0 overflow-y-auto pr-2 scrollbar-hide">
        ...
        <div className="flex-1 bg-white border border-gray-100 rounded-2xl p-4 sm:p-6 shadow-sm flex flex-col min-w-0">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 shrink-0">
            <h4 className="text-xl font-bold text-gray-800 flex items-center gap-4">
            ...
            {calendarCells.map((day, idx) => {
              ...
              return (
                <button
                  ...
                  className={`bg-white min-h-[90px] p-2 flex flex-col items-start transition-all relative border-b border-r border-gray-100 ${
                    isSelected ? 'ring-2 ring-inset ring-[var(--color-primary)] bg-blue-50/30' : ''
                ...
                >
                  <span className={`text-sm font-bold w-8 h-8 flex items-center justify-center rounded-full ${
                    isToday ? 'bg-[var(--color-primary)] text-white' : isSelected ? 'text-[var(--color-primary)]' : 'text-gray-700'
                  ...
                  {hasEvents && !isSelected && (
                    <div className="mt-2 w-full px-1">
                      <div className="w-full bg-[#fdf0f3] text-[#d65b79] text-[10px] font-bold p-1 rounded mb-1 truncate">3 Available</div>
                    </div>
                  )}
                  {isUnavailable && (
                    <div className="mt-2 w-full px-1">
                      <div className="w-full bg-gray-200 text-gray-500 text-[10px] font-bold p-1 rounded mb-1 truncate shadow-sm">Unavailable</div>
                    </div>
                  )}
                  {isSelected && (
                    <div className="mt-2 w-full px-1">
                      <div className="w-full bg-[var(--color-primary)] text-white text-[10px] font-bold p-1 rounded mb-1 truncate shadow-sm">Selected</div>
                    </div>
                  )}
                </button>
              );
        ...
        <div className="w-full xl:w-[320px] shrink-0 flex flex-col gap-6">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
            <label className="block text-sm font-bold text-gray-800 mb-4">Consultation Type</label>
                ...
                {timeSlots.length === 0 ? (
                  ...
                  timeSlots.map((time) => (
                    <button
                      ...
                      className={`py-3 px-3 text-sm font-semibold rounded-xl transition-all ${
                        data.timeSlot === time
