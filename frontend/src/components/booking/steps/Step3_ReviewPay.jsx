...
import { FaCalendarAlt, FaClock, FaUser, FaStethoscope, FaMapMarkerAlt, FaNotesMedical, FaQrcode, FaCreditCard, FaUniversity, FaCheckCircle, FaSpinner, FaLock } from 'react-icons/fa';
import { useTheme } from '../../../theme/ThemeProvider';
export const Step3_ReviewPay = ({
  data,
  onSuccess,
  prev
}) => {
  ...
  if (isProcessing) {
    return <div className="flex flex-col items-center justify-center h-full min-h-[400px]">
        ...
        <h3 className="text-2xl  text-gray-800 mb-2">Processing Payment...</h3>
  ...
  return <div className="flex flex-col h-full bg-white rounded-2xl">
      <div className="flex justify-between items-center mb-6 shrink-0">
        <h3 className="text-xl  text-gray-800">Review & Pay</h3>
      </div>
        ...
        <div className="w-full mt-2 flex flex-col gap-8 pb-10">
              ...
              <div className="bg-gray-50/50 rounded p-2 border border-gray-100 shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10 flex gap-4 items-center border-b border-gray-200/60 pb-4 mb-4">
                  ...
                  <div className="flex-1">
                    <h4 className="text-xl  text-gray-800 tracking-tight mb-1">{data.doctor?.name}</h4>
                ...
                <div className="grid grid-cols-2 gap-4 relative z-10">
                  <div className="bg-white p-2 rounded border border-gray-100 shadow-sm flex flex-col justify-center">
                    ...
                    <div className=" text-gray-800 text-sm">{new Date(data.date || '').toLocaleDateString('en-US', {
                      weekday: 'short',
                      month: 'short',
                      day: 'numeric',
                      year: 'numeric'
            ...
            <div className="w-full lg:w-[350px] bg-white border border-gray-200 rounded p-2 shadow-sm h-fit">
              ...
              <div className="flex flex-col gap-3 text-sm">
                <div className="flex justify-between items-center border-b border-gray-50 pb-2">
                  ...
                  <span className=" text-gray-800 text-right">{data.patient?.name}</span>
                </div>
                <div className="flex justify-between items-center border-b border-gray-50 pb-2">
                  <span className="text-gray-500 font-medium">Gender & Age</span>
                  <span className=" text-gray-800 text-right">{data.patient?.gender}, {data.patient?.age} Yrs</span>
                </div>
                <div className="flex justify-between items-center border-b border-gray-50 pb-2">
                  <span className="text-gray-500 font-medium">Mobile Number</span>
                  <span className=" text-gray-800 text-right">+91 {data.mobile}</span>
                </div>
                {data.patient?.description && <div className="pt-1">
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div className="w-full lg:w-[320px] bg-gradient-to-br from-gray-800 to-gray-900 rounded p-2 shadow-lg text-white relative overflow-hidden shrink-0">
              ...
              <h4 className=" text-white mb-6 text-lg relative z-10">Payment Details</h4>
            ...
            {isFree ? <div className="flex-1 w-full bg-white border border-gray-200 rounded shadow-sm flex flex-col overflow-hidden relative">
                ...
                <div className="p-6 flex flex-col items-center justify-center min-h-[250px] text-center">
                  ...
                  <button onClick={handleSimulatePayment} className="w-full max-w-xs py-3 rounded text-white hover:opacity-90 transition-all shadow-sm flex items-center justify-center gap-2 font-semibold" style={{
                backgroundColor: tenant?.theme?.primaryColor || '#0d9488'
              }}>
                    Confirm Booking
                  </button>
                </div>
              </div> : <div className="flex-1 w-full bg-white border border-gray-200 rounded shadow-sm flex flex-col overflow-hidden relative">
                <div className="bg-gray-50 border-b border-gray-200 p-4 flex justify-between items-center">
                  <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col sm:flex-row h-full">
                  ...
                  <div className="w-full sm:w-48 bg-gray-50/50 border-r border-gray-100 flex flex-row sm:flex-col p-2 gap-1 overflow-x-auto">
                    <button onClick={() => setMethod('upi')} className={`flex items-center gap-2 p-3 text-sm  rounded whitespace-nowrap transition-all ${method === 'upi' ? 'bg-white shadow-sm text-[var(--color-primary)] border border-gray-200' : 'text-gray-600 hover:bg-gray-100 border border-transparent'}`}>
                      <FaQrcode /> UPI / QR
                    </button>
                    <button onClick={() => setMethod('card')} className={`flex items-center gap-2 p-3 text-sm  rounded whitespace-nowrap transition-all ${method === 'card' ? 'bg-white shadow-sm text-[var(--color-primary)] border border-gray-200' : 'text-gray-600 hover:bg-gray-100 border border-transparent'}`}>
                      <FaCreditCard /> Cards
                    </button>
                    <button onClick={() => setMethod('netbanking')} className={`flex items-center gap-2 p-3 text-sm  rounded whitespace-nowrap transition-all ${method === 'netbanking' ? 'bg-white shadow-sm text-[var(--color-primary)] border border-gray-200' : 'text-gray-600 hover:bg-gray-100 border border-transparent'}`}>
                  ...
                  <div className="flex-1 p-6 flex flex-col items-center justify-center min-h-[250px]">
                    {method === 'upi' && <div className="flex flex-col items-center w-full max-w-xs text-center">
                        ...
                        <button onClick={handleSimulatePayment} className="w-full py-3 rounded bg-[#22c55e] text-white  hover:bg-[#1ea34d] transition-all shadow-sm flex items-center justify-center gap-2">
                          <FaCheckCircle /> Simulate Payment
                        </button>
                      </div>}

                    {method === 'card' && <div className="w-full max-w-xs flex flex-col items-center text-center">
                    ...
                    {method === 'netbanking' && <div className="w-full max-w-xs flex flex-col items-center text-center">
      ...
      <div className="mt-4 pt-6 border-t border-gray-100 flex justify-start shrink-0">
        <button onClick={prev} className="p-2 rounded  text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors">
          Back
        </button>
      </div>
    </div>;
