...
export const Step1_SelectSlot = ({
  ...
  useEffect(() => {
    if (tenant && data.doctor) {
      db.getAppointments(tenant.id, data.doctor.id).then(setAllAppointments).catch(err => {
  ...
  const bookedSlots = data.date ? allAppointments.filter(a => a.date === data.date).map(a => a.time_slot) : [];
  ...
  const timeSlots = data.type === 'Online' ? data.doctor?.onlineHours || [] : data.doctor?.inPersonHours || [];
  const handleNext = () => {
  ...
  return <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex flex-col xl:flex-row gap-2 flex-1 min-h-0 overflow-y-auto pr-2 scrollbar-hide">
        ...
        <div className="flex-1 bg-white border border-gray-100 rounded p-2 sm:p-2  flex flex-col min-w-0">
          ...
          <div className="grid grid-cols-7 gap-px bg-gray-100 rounded overflow-hidden border border-gray-100 flex-1">
            {/* Day Headers */}
            {dayNames.map(day => <div key={day} className="bg-gray-50 p-2 text-center text-sm font-semibold text-gray-500">
                {day}
              </div>)}
            ...
            const totalSlots = (data.type === 'Online' ? data.doctor?.onlineHours || [] : data.doctor?.inPersonHours || []).length;
            ...
            return <button key={day} disabled={isUnavailable || isBookedOut || isNoSlots} onClick={() => updateData({
              date: dateStr,
              timeSlot: null
            })} // Reset time slot when changing date
            className={`bg-white min-h-[75px] p-1.5 flex flex-col items-start transition-all relative border-b border-r border-gray-100 ${isSelected ? 'ring-2 ring-inset ring-[var(--color-primary)] bg-blue-50/30' : ''} ${isUnavailable || isBookedOut || isNoSlots ? 'opacity-40 cursor-not-allowed bg-gray-50' : 'hover:bg-gray-50'}`}>
                  <span className={`text-sm  w-8 h-8 flex items-center justify-center rounded-full ${isToday ? 'bg-[var(--color-primary)] text-white' : isSelected ? 'text-[var(--color-primary)] font-semibold' : 'text-gray-700'} ${isUnavailable ? 'line-through text-gray-400' : ''}`}>
                    {day}
                  </span>

                  {!isUnavailable && !isSelected && hasSlots && <div className="mt-1 w-full">
                      <div className="w-full bg-emerald-50 text-emerald-600 text-[10px] p-0.5 rounded font-semibold text-center truncate">
                        {availableCount} Available
                      </div>
                    </div>}
                  {!isUnavailable && !isSelected && isBookedOut && <div className="mt-1 w-full">
                      <div className="w-full bg-red-50 text-red-500 text-[10px] p-0.5 rounded font-semibold text-center truncate">
                        Booked Out
                      </div>
                    </div>}
                  {!isUnavailable && !isSelected && isNoSlots && <div className="mt-1 w-full">
                      <div className="w-full bg-gray-50 text-gray-400 text-[10px] p-0.5 rounded font-semibold text-center truncate">
                        No Slots
                      </div>
                    </div>}
                  {isUnavailable && <div className="mt-1 w-full">
                      <div className="w-full bg-gray-100 text-gray-400 text-[10px] p-0.5 rounded font-semibold text-center truncate ">
                        Unavailable
                      </div>
                    </div>}
                  {isSelected && <div className="mt-1 w-full">
        ...
        <div className="w-full xl:w-[320px] shrink-0 flex flex-col gap-6">
          ...
          <div className="bg-white border border-gray-100 rounded p-3 ">
            ...
            <div className="flex flex-col gap-3">
              ...
              type: 'In-Person'
            })} className={`p-2 border rounded flex items-center gap-3 transition-all ${data.type === 'In-Person' ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/5 text-[var(--color-primary)] ring-1 ring-[var(--color-primary)]' : 'border-gray-200 hover:border-gray-300 text-gray-600'}`}>
              ...
              <button onClick={() => updateData({
              type: 'Online'
            })} className={`p-2 border rounded flex items-center gap-3 transition-all ${data.type === 'Online' ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/5 text-[var(--color-primary)] ring-1 ring-[var(--color-primary)]' : 'border-gray-200 hover:border-gray-300 text-gray-600'}`}>
          ...
          <div className="bg-white border border-gray-100 rounded p-3  flex-1">
            <label className="block text-sm  text-gray-800 mb-4">
              Time Slot {data.date && <span className="text-[var(--color-primary)] ml-1">({new Date(data.date).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric'
              })})</span>}
            </label>

            {!data.date ? <div className="h-40 flex items-center justify-center text-sm text-gray-400 text-center border-2 border-dashed border-gray-100 rounded">
            ...
            opacity: 1
                ...
                {timeSlots.length === 0 ? <div className="col-span-2 text-sm text-gray-500 text-center py-4 bg-gray-50 rounded border border-gray-100">
                    No slots available for this consultation type.
                  </div> : timeSlots.map(time => {
              const isBooked = bookedSlots.includes(time);
              return <button key={time} disabled={isBooked} onClick={() => updateData({
                timeSlot: time
              })} className={`p-2 text-sm font-semibold rounded transition-all ${isBooked ? 'bg-gray-100 text-gray-400 cursor-not-allowed opacity-50' : data.timeSlot === time ? 'bg-[var(--color-primary)] text-white shadow-md' : 'bg-gray-50 border border-gray-100 hover:border-[var(--color-primary)] hover:bg-blue-50/30 text-gray-700'}`}>
      ...
      <div className="mt-2 border-t border-gray-100 flex justify-end">
        <button onClick={handleNext} disabled={!isComplete} className={`p-2 rounded  transition-all text-sm ${isComplete ? 'bg-[var(--color-primary)] text-white hover:opacity-90 shadow-md' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>
          Continue
        </button>
      </div>
    </div>;
