...
export const DoctorManager = () => {
  ...
  return <div className="min-h-screen bg-gray-50 text-gray-800 p-8">
      <div className="max-w-6xl mx-auto">
        <button onClick={() => navigate('/admin')} className="flex items-center gap-2 text-gray-500 hover:text-gray-800 font-medium mb-6 transition-colors">
        ...
        <div className="flex justify-between items-end mb-8">
          ...
          <button onClick={() => setIsAdding(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white p-2 rounded  text-xs flex items-center gap-2 shadow-md transition-colors">
            <FaPlus /> Add Doctor
          </button>
        </div>

        {isAdding && <form onSubmit={handleAddDoctor} className="bg-white rounded p-4 border border-gray-100 mb-8 grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input required type="text" placeholder="Dr. John Doe" value={formData.name} onChange={e => setFormData({
            ...formData,
            name: e.target.value
            ...
            <div className="flex flex-col gap-2">
              ...
              <input required type="text" placeholder="Cardiologist" value={formData.speciality} onChange={e => setFormData({
            ...formData,
            ...
            <div className="flex flex-col gap-2">
              ...
              <input type="text" placeholder="https://..." value={formData.photo} onChange={e => setFormData({
            ...formData,
            ...
            <div className="flex flex-col gap-2">
              ...
              <input type="text" placeholder="10 Years" value={formData.experience} onChange={e => setFormData({
            ...formData,
            ...
            <div className="flex flex-col gap-2">
              ...
              <input type="email" placeholder="doctor@example.com" value={formData.email} onChange={e => setFormData({
            ...formData,
            ...
            <div className="flex flex-col gap-2">
              ...
              <input type="number" min="0" placeholder="0 (Leave empty or 0 for free)" value={formData.fee || ''} onChange={e => setFormData({
            ...formData,
            ...
            <div className="flex flex-col gap-2 col-span-full mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {STANDARD_SLOTS.map(slot => <button key={slot} type="button" onClick={() => {
              const current = formData.inPersonHours || [];
              const isSelected = current.includes(slot);
              setFormData({
                ...formData,
                inPersonHours: isSelected ? current.filter(s => s !== slot) : [...current, slot]
              });
            ...
            <div className="flex flex-col gap-2 col-span-full">
              ...
              setFormData({
                ...formData,
                onlineHours: isSelected ? current.filter(s => s !== slot) : [...current, slot]
              });
            ...
            <div className="flex flex-col gap-2 col-span-full">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {formData.unavailableDates?.map(date => <div key={date} className="flex items-center gap-1 bg-red-50 text-red-600 px-3 py-1 rounded-full text-xs  border border-red-100">
                    {date}
                ...
                {(!formData.unavailableDates || formData.unavailableDates.length === 0) && <span className="text-xs text-gray-400 font-medium">No unavailable dates configured.</span>}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {doctors.map(doctor => <div key={doctor.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col items-center text-center relative group">
              ...
              <button onClick={() => navigate(`/admin/appointments?tenant_id=${id}&doctor_id=${doctor.id}`)} className="mt-4 w-full py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg text-xs font-semibold flex items-center justify-center gap-2 transition-colors border border-indigo-50">
                <FaCalendarAlt /> Booked Calendar
              </button>
            </div>)}

          {doctors.length === 0 && !isAdding && <div className="col-span-full py-12 text-center text-gray-500">
