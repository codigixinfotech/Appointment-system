...
export const AppointmentsCalendar = () => {
  ...
  const renderMonthView = () => <div className="grid grid-cols-7 gap-px bg-gray-200 border border-gray-200 rounded overflow-hidden mt-4">
      ...
      return <div key={day} className={`bg-white min-h-[120px] p-2 flex flex-col border-b border-r border-gray-100 ${isToday ? 'bg-blue-50/20' : ''}`}>
            ...
            <div className="flex flex-col gap-1 overflow-y-auto max-h-[80px] scrollbar-hide">
              {dayAppts.map(appt => <button key={appt.id} onClick={() => setSelectedAppointment(appt)} className="text-left bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs p-1.5 rounded truncate border border-indigo-100 transition-colors flex flex-col gap-0.5">
                  <span className="truncate"><span className="font-semibold">{appt.time_slot}</span> - {appt.patient_name}</span>
                  {!selectedDoctor && appt.doctor && <span className="text-[9px] text-indigo-500 font-medium truncate">Dr. {appt.doctor.name}</span>}
                </button>)}
            </div>
          </div>;
    })}
  ...
  const renderDayView = () => {
    ...
    return <div className="mt-4 flex flex-col gap-3">
        {dayAppts.length === 0 ? <div className="bg-gray-50 border border-gray-100 rounded p-12 text-center text-gray-500">
            No appointments scheduled for this date.
          </div> : dayAppts.map(appt => <div key={appt.id} onClick={() => setSelectedAppointment(appt)} className="bg-white border border-gray-100 rounded p-4 shadow-sm hover:shadow-md transition-all cursor-pointer flex gap-4 items-center">
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex justify-between items-start">
                  ...
                  {!selectedDoctor && appt.doctor && <span className="text-xs bg-indigo-50 text-indigo-700 px-2.5 py-0.5 rounded-full font-semibold">Dr. {appt.doctor.name}</span>}
                </div>
  ...
  return <div className="min-h-screen bg-gray-50 text-gray-800 p-8">
      <div className="max-w-7xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <select value={selectedTenant} onChange={e => setSelectedTenant(e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-700 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-64 p-2.5 outline-none">
                <option value="" disabled>Select a Hospital</option>
                {tenants.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
              </select>

              {selectedTenant && <select value={selectedDoctor} onChange={e => setSelectedDoctor(e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-700 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-64 p-2.5 outline-none">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-4 bg-gray-50 px-4 py-2 rounded border border-gray-100">
              ...
              <div className="w-36 text-center font-bold text-gray-800">
                {view === 'month' ? `${monthNames[month]} ${year}` : currentDate.toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric'
              })}
            ...
            <div className="flex bg-gray-100 p-1 rounded">
              <button onClick={() => setView('month')} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${view === 'month' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                Month
              </button>
              <button onClick={() => setView('day')} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${view === 'day' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                Day
              </button>
            </div>
          </div>
        ...
        {/* Main Calendar View */}
        {loading ? <div className="flex items-center justify-center h-64 text-gray-400">Loading appointments...</div> : view === 'month' ? renderMonthView() : renderDayView()}

      </div>

      {/* Appointment Details Modal */}
      {selectedAppointment && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              {selectedAppointment.doctor && <div className="flex justify-between border-b border-gray-100 pb-2">
              ...
              {selectedAppointment.patient_email && <div className="flex justify-between border-b border-gray-100 pb-2">
              ...
              {selectedAppointment.patient_description && <div className="pt-2">
